...
import { TIMEFRAMES, TIMEFRAME_KEYS, getTimeframeConfig, APP_CONFIG } from './src/config.js';
...
function printGoals(goals, timeframe) {
...
function calculateProgress(goals) {
    ...
    if (total === 0)
        return { bar: chalk.dim('░'.repeat(10)), text: 'No goals', percent: 0 };
...
// --- DATA ACCESS ---
async function getLog(timeframe) {
    const config = getTimeframeConfig(timeframe);
...
async function createLog(timeframe, goals, obstacle) {
    const config = getTimeframeConfig(timeframe);
    const docId = getDocId(timeframe);
    const docRef = db.collection(config.collection).doc(docId);
    const payload = {
...
// --- PROMPTS ---
async function promptForGoals(timeframe) {
    const goals = [];
    let goalCount = 1;
    const theme = CHALK_THEMES[timeframe];
    const config = getTimeframeConfig(timeframe);
    ...
    while (true) {
        ...
        if (goal.trim().length === 0 && goalCount > 1)
            break;
...
async function promptForObstacle() {
...
// --- GOAL ACTIONS ---
async function toggleGoal(docRef, data) {
...
async function addGoal(docRef, data) {
...
async function editGoal(docRef, data) {
    if (data.goals.length === 0) {
        console.log(chalk.yellow('\n⚠ No goals to edit'));
        return;
    }
...
async function removeGoal(docRef, data) {
    if (data.goals.length === 0) {
        console.log(chalk.yellow('\n⚠ No goals to remove'));
        return;
    }
...
async function writeReflection(docRef, data) {
...
async function setObstacle(docRef, data) {
    const obstacle = await input({
        message: chalk.yellow('⚠️  What obstacle do you anticipate?'),
...
// --- MENUS ---
async function showTimeframeMenu(timeframe) {
    const theme = CHALK_THEMES[timeframe];
    ...
    while (true) {
        ...
        switch (action) {
            case 'toggle':
                await toggleGoal(docRef, data);
                break;
            case 'add':
                await addGoal(docRef, data);
                break;
            case 'edit':
                await editGoal(docRef, data);
                break;
            case 'remove':
                await removeGoal(docRef, data);
                break;
            case 'obstacle':
                await setObstacle(docRef, data);
                break;
            case 'reflect':
                await writeReflection(docRef, data);
                break;
            case 'exit':
...
async function showMainMenu() {
    while (true) {
        ...
        const summaries = TIMEFRAME_KEYS.map((tf, i) => ({ timeframe: tf, data: results[i].data }));
        ...
        if (!TIMEFRAME_KEYS.includes(timeframe)) {
            continue;
        }
        await showTimeframeMenu(timeframe);
    }
}
// --- QUICK PREVIEW ---
async function showQuickPreview(summaries) {
    printHeader();
