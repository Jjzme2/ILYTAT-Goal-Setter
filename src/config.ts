...
export interface TimeFrameConfig {
    ...
    collection: string;
    description: string;
    color: string; // Hex color for PWA
}
...
export function getTimeframeConfig(key: TimeFrame): TimeFrameConfig {
    const config = TIMEFRAMES.find(t => t.key === key);
    if (!config) throw new Error(`Unknown timeframe: ${key}`);
    return config;
}

// All valid timeframe keys
export const TIMEFRAME_KEYS: TimeFrame[] = TIMEFRAMES.map(t => t.key);
